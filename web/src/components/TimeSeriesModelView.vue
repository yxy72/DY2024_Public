...
const props = defineProps<{
  ...
  state: any,
  menuRoute: string,
  loadModelBeforeMount?: boolean,
  sendTrainingStartAfterState?: boolean,
}>()
const store = useStore()
const modelName:string = props.modelName;
let d = reactive(props.state);
let pd = reactive({
  modelInfo:[
...
onUnmounted(() => {
  window.onresize = () => {}
  store.state.status.menu[1].route = props.menuRoute;
})
...
const TRAIN = ()=>{
  ...
  rPush("开始训练...")
  if (!props.sendTrainingStartAfterState) {
